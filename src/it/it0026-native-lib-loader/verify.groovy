...
List<File> entries = [new File(localRepositoryPath,
    'org/scijava/native-lib-loader/2.0.2/native-lib-loader-2.0.2.jar')]
new File(basedir, 'target').eachFile { File file ->
  if (file.name.startsWith('it0026-native-lib-loader-1.0-SNAPSHOT') && file.isFile()) {
    entries.add(file)
  }
}
assert entries.size() > 1 : 'No packaged NAR artifacts found'
